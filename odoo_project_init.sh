...
do
        ...
        conf_project=`./conf_gen.sh $admin_passwd 5432 $usr $db_passwd $addons_path "/opt/odoo$ver/projects/$i/data" $odoo_port $odoopoll_port $usr 2 1 2147483648 2147483648 8192 3600 3600 ${pid_file} ${log_file} ${log_level}`
        sudo echo $conf_project > /etc/odoo/$i/$i.conf
        serv_conf=`./create_service_odoo.sh $i`
		sudo echo $serv_conf > /etc/systemd/system/$i.service
done;
